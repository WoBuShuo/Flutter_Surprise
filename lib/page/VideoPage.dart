...
class VideoState extends State<VideoPage> {
  ...
  void initState() {
    ...
    SecondHttpManager.getInstance().getMovieUrl(_videoUrl, onSuccess: (data) {
      ...
      _controller =
          VideoPlayerController.network(bean.playUrlAndSizeList[0].url)
            ..initialize().then((_) {
              setState(() {
  ...
  void getCommentData() {
    SecondHttpManager.getInstance().moviePlayComment(_videoUrl,
        onSuccess: (data) {
...
//            Image.network(src)
              ...
              Text.rich(TextSpan(children: [
                ...
                TextSpan(
                    text: " • " + RelativeDateFormat.formatMillisecond(item.enterTime),
...
//                  .intoContainer(margin: EdgeInsets.only(left: 10, right: 15)),
      ...
      ).intoContainer(margin: EdgeInsets.only(top: 15,left: 15)));
    }
    return list;
  }
